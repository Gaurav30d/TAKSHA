...
def wishMe():
    ...
    ui.terminalPrintAndSay("I am TAKSHA Sir....")
...
def main():
    ...
    while not exit:
        ...
        if 'chat mode' in query or 'chat' in query:
            ...
            retry += 1
            if(retry  > 4):
                ui.terminalPrintAndSay("Taksha entering sleep mode")
                listen_for_wake_word()
                ui.terminalPrintAndSay('Taksha is awake')
                retry=0
            else:
...
def quit_jarvis():
    ui.terminalPrintAndSay("Taksha is Exiting")
